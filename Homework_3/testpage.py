...
class TestSearchLocators:
    ...
    LOCATOR_CREATE_BUTTON = (
        By.XPATH, '//*[@id="app"]/main/div/div[2]/div[1]/button')
    LOCATOR_CREATE_TITLE = (
        By.XPATH, """//*[@id="create-item"]/div/div/div[1]/div/label/input""")
    LOCATOR_CREATE_DESCRIPTION = (
        By.XPATH, """//*[@id="create-item"]/div/div/div[2]/div/label/span/textarea""")
    LOCATOR_CREATE_CONTENT = (
    ...
    LOCATOR_RES_CREATE = (
        By.XPATH, """//*[@id="app"]/main/div/div[3]/div[1]/a/h2""")
...
class OperationsHelper(BasePage):
    ...
    def get_login_enter_text(self):
        enter_field = self.find_element(
            TestSearchLocators.LOCATOR_LOGIN_ENTER, time=2)
        text = enter_field.text
        logging.info(
            f'We find text {text} in error field {TestSearchLocators.LOCATOR_LOGIN_ENTER[1]}')
        return text
    ...
    def create_post_title(self, word):
        logging.info(
            f'Send "{word}" to element {TestSearchLocators.LOCATOR_CREATE_TITLE[1]}')
        title_field = self.find_element(
            TestSearchLocators.LOCATOR_CREATE_TITLE)
        title_field.clear()
        title_field.send_keys(word)

    def create_post_description(self, word):
        logging.info(
            f'Send "{word}" to element {TestSearchLocators.LOCATOR_CREATE_DESCRIPTION[1]}')
        description_field = self.find_element(
            TestSearchLocators.LOCATOR_CREATE_DESCRIPTION)
        description_field.clear()
        description_field.send_keys(word)

    def create_post_content(self, word):
        logging.info(
            f'Send "{word}" to element {TestSearchLocators.LOCATOR_CREATE_CONTENT[1]}')
        content_field = self.find_element(
            TestSearchLocators.LOCATOR_CREATE_CONTENT)
        content_field.clear()
    ...
    def get_res_create_text(self):
        enter_field = self.find_element(
            TestSearchLocators.LOCATOR_RES_CREATE, time=2)
        text = enter_field.text
        logging.info(
            f'We find text {text} in error field {TestSearchLocators.LOCATOR_RES_CREATE[1]}')
        return text
    ...
    def contact_us_name(self, word):
        logging.info(
            f'Send "{word}" to element {TestSearchLocators.LOCATOR_YOUR_NAME[1]}')
    ...
    def contact_us_email(self, word):
        logging.info(
            f'Send "{word}" to element {TestSearchLocators.LOCATOR_YOUR_EMAIL[1]}')
    ...
    def contact_us_content(self, word):
        logging.info(
            f'Send "{word}" to element {TestSearchLocators.LOCATOR_YOUR_CONTENT[1]}')
        title_field = self.find_element(
            TestSearchLocators.LOCATOR_YOUR_CONTENT)
        title_field.clear()
        title_field.send_keys(word)
