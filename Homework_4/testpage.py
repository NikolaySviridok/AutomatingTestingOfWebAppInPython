...
class OperationsHelper(BasePage):
    ...
    def click_create_button(self):
        self.click_button(
            TestSearchLocators.ids['LOCATOR_CREATE_BUTTON'], description='Create button')

    def click_create_post_button(self):
        self.click_button(
            TestSearchLocators.ids['LOCATOR_CREATE_POST_BTN'], description='Create post button')

    def click_contact(self):
        self.click_button(
            TestSearchLocators.ids['LOCATOR_CONTACT'], description='contact button')

    def click_contact_us(self):
        self.click_button(
            TestSearchLocators.ids['LOCATOR_CONTACT_US_BTN'], description='contact us button')
...
def get(token):
    try:
        ...
        if resourсe.status_code == 200:
            ...
            logging.error(
                f"Error with retrieving data. Status code: {resourсe.status_code}")
            return None
    except:
...
def post(token):
    try:
        ...
        if new_post.status_code == 200:
            ...
            logging.error(
                f"Error with posting data. Status code: {new_post.status_code}")
            return None
    except:
